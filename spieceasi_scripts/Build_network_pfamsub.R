...
x <- data.frame(M=apply(pp_tr,2,mean,na.rm=T),
                CV=apply(pp_tr,2,co.var))
ggplot(x,aes(x=M,y=CV)) + geom_point(alpha=0.2) + geom_density2d() +
  scale_x_log10() + scale_y_log10()


# hist(colMeans(pp_tr),breaks=1000,xlim=c(0,500))
# hist(apply(pp_tr, 2, function(x) max(x, na.rm = TRUE)),breaks=1000,xlim=c(0,1000))
# sum(colMeans(pp_tr)>50)
# pp_tr <- pp_tr[,apply(pp_tr, 2, function(x) max(x, na.rm = TRUE))>300]
...
save(pp.net,pp.net.ig,file="pfam_net_CV200.RData")
